...
class AnyMathsAdapter(GEPAAdapter[AnyMathsDataInst, AnyMathsTrajectory, AnyMathsRolloutOutput]):
    ...
    def evaluate(
        ...
        for data, response in zip(batch, responses, strict=False):
            ...
            if correct_output_format and isinstance(assistant_response, dict):
                ...
                output: AnyMathsRolloutOutput = {"full_assistant_response": structured_assistant_response}
                score = 1.0 if data["answer"] in assistant_response.get("final_answer", "") else self.failure_score
            else:
                output: AnyMathsRolloutOutput = {"full_assistant_response": str(assistant_response)}
