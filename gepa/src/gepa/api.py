...
from gepa.strategies.component_selector import RoundRobinReflectionComponentSelector, AllComponentsReflectionComponentSelector
...
def optimize(
    ...
    if adapter is None:
        ...
        adapter = cast(Any, DefaultAdapter(model=task_lm))
    else:
    ...
    if isinstance(reflection_lm, str):
        ...
        def _call(prompt: str) -> str:
            resp = litellm.completion(model=reflection_lm_name, messages=[{"role": "user", "content": prompt}])
    ...
    # Use an all-components selector so each proposal can co-adapt multiple components together
    module_selector = AllComponentsReflectionComponentSelector()
    batch_sampler = EpochShuffledBatchSampler(minibatch_size=reflection_minibatch_size, rng=rng)

    # Create a non-optional, broadly-typed adapter reference for type checking
    adapter_nn = cast(GEPAAdapter[Any, Any, Any], adapter)
    ...
    def evaluator(inputs, prog):
        eval_out = adapter_nn.evaluate(inputs, prog, capture_traces=False)
