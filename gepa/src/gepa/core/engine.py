...
class GEPAEngine(Generic[DataInst, Trajectory, RolloutOutput]):
    ...
    def _val_evaluator(self) -> Callable[[dict[str, str]], tuple[list[RolloutOutput], list[float]]]:
        if self.valset is None:
            raise ValueError("valset must not be None when calling _val_evaluator")
        assert self.valset is not None  # for type checker
        from typing import cast
        return lambda prog: self.evaluator(cast(list[DataInst], self.valset), prog)
